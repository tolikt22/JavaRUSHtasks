...
same - (1, 1) - (4, 1)
        ...
        for (int k = 0; k < w.length; k++) {                 //перебираем по слову
            ...
            for (int i = 0; i < crossword.length; i++) {
                for (int j = 0; j < crossword[i].length; j++) {
                    if (crossword[i][j] == chars[0]) {                        //если первая буква слова совпадает c коорд. массива
    ...
    private static Word detectDirections(int x, int y, char[] word, int[][] crossword) {  //ищем слово во всех напр.
        ...
        for (int direction = 1; direction <= 8; direction++) {    // перебор направлений
            StringBuilder wordString = new StringBuilder();
            int endX, endY;
            int count = 0;                                        //matching chars counter
            int charCount = 0;                                    //in word chars counter
            x = tmpX;
            y = tmpY;                                             //keeping array borders
            while (y >= 0 && x >= 0 && y < crossword.length && x < crossword[0].length && charCount < word.length) {
                if (word[charCount] == crossword[y][x]) {         // checks char matching
